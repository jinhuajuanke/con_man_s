...
class TypeLinkm
{
    ...
    {
        global $dedemao_path;
        $indexpage = "<a href='{$dedemao_path}/index.html'>".$this->indexName."</a>";
    ...
    {
        global $dedemao_path,$dedemao_usetype,$dedemao_typerule;
        //栏目规则
        if($dedemao_usetype==0 && $dedemao_typerule){
            $url = GetTypeUrl($typeinfos['id'],MfTypedir($typeinfos['typedir']),1,$typeinfos['defaultname'],
                $typeinfos['ispart'],$dedemao_typerule,$typeinfos['moresite'],$typeinfos['siteurl'],$typeinfos['sitepath']);
            $dedemao_path = rtrim($dedemao_path,'/') ? rtrim($dedemao_path,'/') : '/m';
            $typedir = str_replace("{cmspath}","",$url);
            $url = $dedemao_path.$typedir;
        }else{
            $url = GetTypeUrl($typeinfos['id'],MfTypedir($typeinfos['typedir']),$typeinfos['isdefault'],$typeinfos['defaultname'],
