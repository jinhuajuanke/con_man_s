...
require_once('typelinkm.class.php');
@set_time_limit(0);
/**
 * 主文档类(Archives类)
 *
...
class ArchivesMobile
{
    ...
    function __construct($aid)
    {
        ...
        else
        {
            ...
            $this->TypeLink = new TypeLinkm($arr['typeid']);
    ...
    {
        global $cfg_remote_site,$fileFirst,$dedemao_path,$dedemao_arcrule,$dedemao_usearc;
        if($this->IsError)
        {
            return '';
        }
        ...
        //文档命名规则
        $dedemao_path = rtrim($dedemao_path,'/') ? rtrim($dedemao_path,'/') : '/m';
        if($dedemao_usearc==0 && $dedemao_arcrule){
            $this->TypeLink->TypeInfos['namerule'] = $dedemao_path.$dedemao_arcrule;
        }else $this->TypeLink->TypeInfos['namerule'] = $dedemao_path.$this->TypeLink->TypeInfos['namerule'];
        //分析要创建的文件名称
    ...
    function ReplaceKeyword($kw,&$body)
    {
        ...
        */
        ...
        {
            $body = @preg_replace_callback("#(^|>)([^<]+)(?=<|$)#sU", "_highlightm('\\2', \$karr, \$kaarr, '\\1')", $body);
        } else {
            $body = @preg_replace("#(^|>)([^<]+)(?=<|$)#sUe", "_highlightm('\\2', \$karr, \$kaarr, '\\1')", $body);
        }
...
//高亮专用, 替换多次是可能不能达到最多次
function _highlightm($string, $words, $result, $pre)
{
